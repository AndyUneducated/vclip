...
from .merge import _resolve_inputs, plan_merge
from .probe import ProbeError, format_size, probe
from .split import plan_duration, plan_size, plan_trim
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_trim.add_argument("--dry-run", action="store_true",
                        help="只打印将执行的命令，不实际运行")
    p_trim.add_argument("--yes", "-y", action="store_true", help="跳过确认直接执行")
    ...
    p_merge.add_argument("--dry-run", action="store_true",
                         help="只打印将执行的命令，不实际运行")
    p_merge.add_argument("--yes", "-y", action="store_true", help="跳过确认直接执行")
...
def _confirm_and_run(plan, *, dry_run: bool, yes: bool, verify_fn=None) -> int:
    print(plan.describe())
    ...
    files = plan.execute(dry_run=dry_run)
    if dry_run:
        print("（dry-run，未实际执行）")
        return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "verify":
            parts = _resolve_inputs(args.parts)
